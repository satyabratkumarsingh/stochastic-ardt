    ...
    if env_name == "kuhn_poker":
        ...
        if current_player_turn == 0: # It's agent's turn
            ...
            if not (terminated or truncated) and env.player_turn == 1:
                ...
                print(f"  --- Worst-Case Wrapper: Forcing Adversary action: {adv_action_to_take} ---")
...
def evaluate_episode(
    ...
    # --- START EPISODE PRINT ---
    print(f"\n--- Starting Episode for Target Return: {target_return:.4f} ---")
    ...
    for t in range(max_ep_len):
        ...
        if action_type == 'discrete':
            ...
            # --- PRINT STATEMENT 1: Action probabilities and chosen action (discrete) ---
            print(f"  Timestep {t}:")
            print(f"    Action Probabilities: {act_probs.cpu().detach().numpy()}") # .detach() added for safety
            print(f"    Chosen Action Index: {action_idx.item()}")
            # --- END PRINT STATEMENT 1 ---
        else:
            one_hot_action = action.unsqueeze(0).unsqueeze(0)  # (1, 1, act_dim)
            action_for_step = action.detach().cpu().numpy()
            
            # --- PRINT STATEMENT 1: Chosen action (continuous) ---
            print(f"  Timestep {t}:")
            print(f"    Chosen Action (continuous): {action_for_step}")
            # --- END PRINT STATEMENT 1 ---

        # Perform environment step
        # --- PRINT STATEMENT 2: Action passed to environment ---
        print(f"    Action passed to env.step(): {action_for_step}")
        # --- END PRINT STATEMENT 2 ---
        ...
        # --- PRINT STATEMENT 3: Results from environment step ---
        print(f"    Reward from env: {reward:.4f}")
        print(f"    Done (terminated/truncated): {done}")
        print(f"    New State (first few elements): {state[:5] if state.size > 5 else state}...") # Print only part of state if large
        print(f"    Info: {infos}")
        # --- END PRINT STATEMENT 3 ---
        ...
        # --- PRINT STATEMENT 4: Cumulative episode return ---
        print(f"    Cumulative Episode Return: {episode_return:.4f}")
        print(f"    Current Return-to-go (for next step): {returns_to_go_history[0, -1].item():.4f}")
        # --- END PRINT STATEMENT 4 ---

        if done:
            # --- PRINT STATEMENT 5: Episode termination ---
            print(f"  Episode terminated at Timestep {t + 1}.")
            break

    # --- FINAL EPISODE SUMMARY PRINT ---
    print(f"--- Episode Finished. Total Episode Return: {episode_return:.4f}, Length: {episode_length} ---")
    # --- END FINAL EPISODE SUMMARY PRINT ---
