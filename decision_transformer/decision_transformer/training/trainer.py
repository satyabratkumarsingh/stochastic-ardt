...
class Trainer:
    ...
    def _get_batch(self, device: str = "cpu"):
        print(f"Batch size: {self.train_configs.batch_size}")
        ...
        for i in range(self.train_configs.batch_size):
            ...
            if self.env_name not in ["halfcheetah", "hopper", "walker2d"]:
                ...
                si = np.random.randint(0, len(traj['obs']))
        
            # Handle states
            if self.env_name == "connect_four":
                cur_state = np.array([obs for obs in traj['obs']])
            else:
                cur_state = np.array(traj['obs'])
            if cur_state.ndim == 1:
            ...
            tlen = np.sum(np.all(a[-1] != -10, axis=-1))  # Count valid actions
            ...
            timesteps[-1] = np.concatenate([np.zeros((1, self.context_size - s[-1].shape[1])), timesteps[-1]], axis=1)
